            ...
            <div class="col-12">
                <div class="card mb-4">
                    <div class="card-header p-0 mx-3 mt-3 position-relative z-index-1">
                        ...
                        @endphp
                        <h5>{{ $class->name }}</h5>
                        <p class="font-weight-light text-sm">Tanggal: {{ $date }}</p>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('absence.form.submit') }}" method="POST">
                            @csrf
                            <x-fields.select name="teacher_id" label="Guru" :choices="$teachers" :is-enabled="is_null($absence)"
                                :value="$absence?->teacher_id ?? null" />
                            @if (is_null($absence))
                                <input name="date" value="{{ $date }}" type="hidden">
                                <input name="class_schedule_id" value="{{ $schedule['id'] }}" type="hidden">
                        ...
                        <div class="card-body">
                            <div class="row">
                                @foreach ($students as $student)
                                    <form class="col-md-4 col-12" action="{{ route('absence.form.submit') }}"
                                        method="POST">
                                        @csrf
